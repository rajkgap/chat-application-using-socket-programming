...
class Application(tkinter.Tk):
    ...
    def launch_app(self):                                           # default function always run to generate the main 
        self.title('Chat Room')                                     #Name of our Application
    ...
    def reg_menu(self):
        ...
        self.title('Chat Room Registration')
    ...
    def launch_client(self):
        ...
        #MAIN WINDOW
        self.title('Chat Room User: '+self.name)                                                 #Window title
        ...
        self.browsebutton = ttk.Button(self.button_frame,image=self.img1,command = self.browse)  #Button for browsing multimedia
        ...
        #CLIENT FRAME           
        self.user_icon=ttk.Label(self.clients_frame,image=self.img2,background='old lace',text=self.name,compound="top",font=self.font3,anchor=tkinter.E)            #Code for the Display Icon
        self.frame.pack(side = tkinter.TOP, fill = tkinter.BOTH, expand = True)                                                                                   #Packing the above created objects and giving them positions while packing  
        ...
        for i in self.contacts:
            l=ttk.Label(self.f2,padding=10,text=i,justify=tkinter.LEFT,font=self.font3,foreground='gray49',background='ivory2')
            l.grid(row=j,column=0,sticky=tkinter.W)
        ...
        self.clientchat_thread = threading.Thread(name = 'clientchat', target = self.clientchat) #for client we will intiate a thread to display chat
    ...
    def user_selection(self):   
        ...
        label1=tkinter.Label(frame1,text="Select the users you want to connect to:",compound=tkinter.LEFT,font=('Helvetica','20'),justify=tkinter.CENTER)
        ...
        for client in self.list_of_active_users:
            ch=tkinter.Checkbutton(frame2,text=client,variable=self.enable[client],borderwidth=0,pady=10,justify=tkinter.LEFT,font=('Courier New',19),foreground='gray30')
            ch.grid(column=0,row=i,sticky=tkinter.W)
            i=i+1
        frame3=tkinter.Frame(frame2,borderwidth=10)
        b=tkinter.Button(frame3,text="Connect",justify=tkinter.CENTER,font=("Helvetica",'14'),padx=6,pady=6,command=self.root.withdraw)
    ...
    def add_contact(self):
        ...
        label1=tkinter.Label(self.root,text="Enter the Contact Details:",font=('Helvetica',20),justify=tkinter.CENTER,pady=10)
    ...
    def add(self,event):
        ...
        l=ttk.Label(self.f2,padding=10,text=self.contact.get(),justify=tkinter.LEFT,font=self.font3,foreground='gray49',background='ivory2')            # l.grid(row=self.num_contacts,column=0,sticky=tkinter.W)
    ...
    def del_contact(self):
        ...
        label1=tkinter.Label(self.root,text="Enter the Contact to be removed:",font=('Helvetica',20),justify=tkinter.CENTER,pady=10)
    ...
    def save_history(self):
        self.save_file=tkinter.filedialog.asksaveasfilename(title="Choose save location",filetypes=[('Plain text','*.txt'),('Any File','*.*')])
        try:
    ...
    def send(self,event):
        ...
        self.chat_text.tag_config('tag{0}'.format(1),justify=tkinter.RIGHT,foreground='RoyalBlue3',font=self.chat_font)
