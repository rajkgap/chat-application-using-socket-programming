...
def authenticate(uname, pwd):
    ...
    if  uname in USERS_LIST:					#If username is the same as one of the active users authentication becomes void
       	is_valid=False
       	return is_valid
    with open('textfiles/database.txt','r') as f:			#Checking for the username in the local database
        for line in f:
            if uname+' '+pwd+'\n' == line:
                is_valid = True
                return is_valid
...
def reg_user(uname,pwd):
	...
	flag = 0
	with open('textfiles/database.txt','r') as f:
		for line in f:
			temp = line.split(' ') 
			if uname == temp[0]:			#Indicating that the username is already in local db
				flag=1               

	if flag:								#Unauthenticated registration
		err = 'Error'
		err = err.encode()
		conn.send(err)
	else:
		fo = open('textfiles/database.txt','a')		#Appending the above user detail in local db
		fo.write(uname+' '+pwd+'\n')
		fo.close()
